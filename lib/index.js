...
program
  ...
  .option('--begin-event-name [text]', 'Specify the text that should appear when beginning a new section of task [Begin Task]')
  .option('--break-event-name [text]', 'Specify the text that should appear when taking a break [Take a break]')
...
var duration = parseInt(program.taskDuration || defaults.task_duration),
    ...
    begin_event_name = (program.beginEventName || defaults.begin_event_name),
    break_event_name = (program.breakEventName || defaults.break_event_name);
...
while (task_count < times_to_recur) {
  ...
  if (task_count === times_to_recur) {
    ...
    f.next(function(){
      displayNotification("end", {task_name: task_name, verbose: verbose}, f.slot());
    });
  } else {
